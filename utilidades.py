...
def get_data_extenso(data: str) -> str:
    ...
    return f"{dia} dias do mês de {mes} de {ano}"
...
def destacar_nome_de_guerra(nome_completo):
    # Encontra todas as ocorrências de texto entre chaves
    nomes_destacados = re.findall(r'\{(.*?)\}', nome_completo)

    if nomes_destacados:
        # Concatena os nomes encontrados com um espaço entre eles
        return ' '.join(nomes_destacados)
    else:
        # Retorna o nome completo se não houver nenhum nome destacado
